...
export function Intro({ id, sectionRef, disciplines, scrollIndicatorHidden, ...rest }) {
  ...
  return (
    <Section
      ...
      {...rest}
    >
      <DisplacementSphere className={styles.displacementSphere} />
      <Transition in key={theme.themeId} timeout={3000}>
        {(visible, status) => (
          <Fragment>
            <header className={styles.text}>
              <h1 className={styles.name} data-visible={visible} id={titleId}>
                <DecoderText text="WordPress / Shopify Development Projects" start={visible} delay={500} />
              </h1>
              <Heading level={0} as="h2" className={styles.title}>
                <VisuallyHidden className={styles.label}>{introLabel}</VisuallyHidden>
